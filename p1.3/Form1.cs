    ...
    //double is , not .
{
    public partial class Form1 : Form
    {
        ...
        private int tt_rows =0;//nombre de ligne
        private List<string> Lfournisseur = new List<string>();
        private string user = "";
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            {
                bt_add_user.Visible = true;
                bt_historik.Visible = true;
                bt_sup_user.Visible = true;
            }
            else
            {
                bt_add_user.Visible = false;
                bt_historik.Visible = false;
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    {               
                                        ...
                                        bool test=cc.ajout_pobox(pb, lb_user.Text);//stokage dans bas donne
                                        if (test) {
        ...
        private void tb_montant_KeyPress(object sender, KeyPressEventArgs e)//input just numeric and one ,
        {
            ...
            if ((ch == ',') && ((sender as System.Windows.Forms.TextBox).Text.IndexOf(',') > -1))
            {
                //alow only one , 
                e.Handled = true;
            }
        ...
        private void tb_nComande_KeyPress(object sender, KeyPressEventArgs e)//input only numeric
        {
            ...
            if ((ch == ',') && ((sender as System.Windows.Forms.TextBox).Text.IndexOf(',') > -2))
            {
        ...
        private void dataGridView1_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            lb_poboxp2p.Text= dataGridView1[4, e.RowIndex].Value.ToString();
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        cc.modif_pobox(pb,old_nfact,old_fourni, lb_user.Text);
        ...
        private void bt_export_Click(object sender, EventArgs e)
        {
            ...
            /*
             ...
             {
                 wb.SaveAs(safefiledialog.FileName, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Microsoft.Office.Interop.Excel.XlSaveAsAccessMode.xlExclusive, Type.Missing, Type.Missing, Type.Missing, Type.Missing, Type.Missing);
             }
             */
            //create new xls file 
            cb_fournisseur.Text = cb_fournisseur.SelectedText.ToString();
            string file = "../../../exported/facture("+cb_fournisseur.Text.ToString()+","+tb_nFacture.Text.ToString()+").xlsx";
            Workbook workbook = new Workbook();
            Worksheet worksheet = new Worksheet("First Sheet");
          
            for (i = 0; i < dataGridView1.Columns.Count-1 ; i++)//for the headers
            {
                worksheet.Cells[0, i] =new Cell( dataGridView1.Columns[i+1].HeaderText);
               
            }
            for (i = 1; i < dataGridView1.Rows.Count+1; i++)//for the rows
            {
                for (j = 0; j < dataGridView1.Columns.Count-1; j++)//for the columns
                {
                    worksheet.Cells[i, j] =new Cell(dataGridView1[j+1, i-1].Value.ToString());
                }
            }
        ...
        private void bt_sup_user_Click(object sender, EventArgs e)
        {
            delete_user a = new delete_user();
            if (a.ShowDialog() == DialogResult.OK)
            {
        ...
        private void bt_historik_Click(object sender, EventArgs e)
        {
        ...
        {
            delete_pbcs d = new delete_pbcs(lb_user.Text);
